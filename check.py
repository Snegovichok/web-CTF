...
class Checker(BaseValidator):
    ...
    def _get_info(self):
        ...
        self._log(f'got info: {out}')
        self._fatal(out.startswith("vulns: "), "invalid info format")
    ...
    def run_all(self, step: int):
        ...
        for i, vuln in enumerate(self._vulns, start=1):
            for _ in range(vuln):
                flag = generate_flag(self._name)
                flag_id = self.put(
                    flag=flag, flag_id=secrets.token_hex(16), vuln=i)
